...
template <class T>
Derive<T>::Derive() : u(zero(T())), du(one(T())) {}

template <class T>
Derive<T>::Derive(const T &v) : u(v), du(zero(T())) {}
...
template <class T>
void Derive<T>::set(const T v) { u = v; }

template <class T>
Derive<T> Derive<T>::operator-() const
{
   return Derive<T>(-u, -du);
}

template <class T>
Derive<T> Derive<T>::operator+=(const Derive<T> &r)
{
   return *this = *this + r;
}
...
template <class T>
Derive<T> Derive<T>::operator+=(const T &r)
{
   return *this = *this + r;
}
...
template <class T>
T Derive<T>::df() const { return du; }

template <class T>
T df(const Derive<T> &x) { return x.df(); }
