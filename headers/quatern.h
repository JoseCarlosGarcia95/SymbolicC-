...
template <class T>
Quaternion<T>::Quaternion(T r1, T i1, T j1, T k1)
    : r(r1), i(i1), j(j1), k(k1) {}

template <class T>
Quaternion<T>::Quaternion(const Quaternion<T> &arg)
...
template <class T>
ostream &Quaternion<T>::print(ostream &s) const
{
   s << "(" << r << "," << i << ","
     << j << "," << k << ")";
   return s;
...
template <class T>
istream &Quaternion<T>::input(istream &s)
{
   s >> r >> i >> j >> k;
   return s;
}
...
template <class T>
Quaternion<T> operator*(T factor, Quaternion<T> &arg)
{
   return arg * factor;
}
