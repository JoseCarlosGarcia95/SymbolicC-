...
template <class T>
Vector<T>::Vector(int n) : vector<T>(n) {}
...
template <class T>
Vector<T>::Vector(const Vector<T> &v) : vector<T>(v) {}
...
template <class T>
void Vector<T>::reset(int length, const T &value)
{
...
template <class T>
const Vector<T> &Vector<T>::operator=(const T &value)
{
...
T Vector<T>::operator|(const Vector<T> &v) const
{
   ...
   T result(zero(T()));
   for (int i = 0; i < length; i++)
      result = result + vector<T>::at(i) * v[i];
   return result;
}
...
template <class T>
ostream &Vector<T>::output(ostream &s) const
{
   int lastnum = vector<T>::size();
   for (int i = 0; i < lastnum; i++)
      s << "[" << vector<T>::at(i) << "]" << endl;
   return s;
}
...
template <class T>
istream &Vector<T>::input(istream &s)
{
   int i, num;
