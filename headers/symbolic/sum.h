...
{
    if (s1.type() == typeid(Sum))
        summands = CastPtr<const Sum>(s1)->summands;
    else
...
void Sum::print(ostream &o) const
{
    ...
    for (list<Symbolic>::const_iterator i = summands.begin(); i != summands.end();
         ++i)
    {
        if ((i->type() != typeid(Numeric) || !CastPtr<const Numeric>(*i)->isNegative()) &&
            (i->type() != typeid(Product) || !CastPtr<const Product>(*i)->printsNegative()))
            o << ((i == summands.begin()) ? "" : "+");
        i->print(o);
    }
}
...
Simplified Sum::simplify() const
{
    ...
    // 1-element sum:  (a) -> a
    if (summands.size() == 1)
        return summands.front().simplify();
    ...
    if (!firstm)
        r.summands.push_back(m.simplify());
    ...
    if (!n.isZero())
        r.summands.push_back(n->simplify());
    if (r.summands.size() == 0)
        return Number<int>(0);
    if (r.summands.size() == 1)
        return r.summands.front();
...
int Sum::compare(const Symbolic &s) const
{
    ...
    if (summands.size() != p->summands.size())
        return 0;
    for (i = summands.begin(); i != summands.end(); ++i)
    {
...
PatternMatches Sum::match(const Symbolic &s, const list<Symbolic> &p) const
{
    ...
    if (summands.size() == 0)
        return l;
    if (summands.size() == 1)
        return summands.front().match(s, p);
    if (s.type() != type())
        return l;
    ...
    for (j = thismatch.begin(); j != thismatch.end(); ++j)
    {
        ...
        for (k = j->begin(), i = sum->summands.begin();
            ...
            if (*k)
                s1.summands.push_back(*i);
            else
                s2.summands.push_back(*i);
