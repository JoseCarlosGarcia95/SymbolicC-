...
void Power::print(ostream &o) const
{
   ...
   int parens1 = parameters.front().type() == typeid(Symbol) || parameters.front().type() == typeid(Sin) || parameters.front().type() == typeid(Cos) || parameters.front().type() == typeid(Sinh) || parameters.front().type() == typeid(Cosh) || parameters.front().type() == typeid(Log) || parameters.front().type() == typeid(Derivative);
   int parens2 = parameters.back().type() == typeid(Symbol) || parameters.back().type() == typeid(Sin) || parameters.back().type() == typeid(Cos) || parameters.back().type() == typeid(Sinh) || parameters.back().type() == typeid(Cosh) || parameters.back().type() == typeid(Log) || parameters.back().type() == typeid(Derivative);
   parens1 = !parens1;
   parens2 = !parens2;
   if (parens1)
      o << "(";
