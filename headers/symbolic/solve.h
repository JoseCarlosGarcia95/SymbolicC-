...
Equations solve(const Symbolic& e, const Symbolic& x)
{
    ...
    if (e == 0)
        ...
        Symbolic Q = (3 * a2 - a1 * a1) / 9, R = (9 * a1 * a2 - 27 * a3 - 2 * a1 * a1 * a1) / 54;
        Symbolic S1 = (R + sqrt(Q * Q * Q + R * R)) ^ (Symbolic(1) / 3);
        Symbolic S2 = (R - sqrt(Q * Q * Q + R * R)) ^ (Symbolic(1) / 3);
        soln = (soln, x == S1 + S2 - a1 / 3,
            x == -(S1 + S2) / 2 - a1 / 3 + SymbolicConstant::i * sqrt(Symbolic(3)) * (S1 - S2),
            x == -(S1 + S2) / 2 - a1 / 3 - SymbolicConstant::i * sqrt(Symbolic(3)) * (S1 - S2));
        ...
        if (df(eqi, l.front()) != 0) {
            ...
            for (j = s.begin(); j != s.end(); ++j) {
                ...
                for (k = e.begin(); k != e.end(); ++k)
                    if (k != i)
                        eq.push_back(k->lhs->subst(j->lhs, j->rhs, sc) == k->rhs->subst(j->lhs, j->rhs, sc));
