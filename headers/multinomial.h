...
template <class T>
Multinomial<T>::Multinomial(T c) : variable(""), type(number), n(c) {}
...
int Multinomial<T>::operator==(const Multinomial<T> &p) const
{
  ...
  {
  case number:
    return xy.first.n == xy.second.n;
    break;
  case univariate:
    return xy.first.u == xy.second.u;
    break;
  case multivariate:
    return xy.first.m == xy.second.m;
    break;
  }
  return 0;
}
                          ...
                          const Multinomial<T> &y) const
{
  ...
  {
    if (y.u.empty())
      return make_pair(x, Multinomial<T>(zero(T())));
