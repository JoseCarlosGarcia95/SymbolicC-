...
template <class T>
Rational<T>::Rational() : p(zero(T())), q(one(T())) {}

template <class T>
Rational<T>::Rational(T N) : p(N), q(one(T())) {}

template <class T>
Rational<T>::Rational(T N, T D) : p(N), q(D)
{
   if (D == zero(T()))
   {
...
template <class T>
Rational<T>::Rational(const string &s)
{
   string s1 = s;
...
template <class T>
Rational<T>::Rational(const double &d)
{
...
// Member functions
template <class T>
T Rational<T>::num() const { return p; }

template <class T>
T Rational<T>::den() const { return q; }
...
template <class T>
Rational<T> zero(Rational<T>)
{
   return Rational<T>(zero(T()));
}
