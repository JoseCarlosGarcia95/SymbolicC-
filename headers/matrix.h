...
template <class T>
T tr(const Matrix<T> &m) { return m.trace(); }
template <class T>
T det(const Matrix<T> &m) { return m.determinant(); }
...
template <class T>
void Matrix<T>::resize(int r, int c, const T &value)
{
   mat.resize(r);
   for (int i = 0; i < r; i++)
      mat[i].resize(c, value);
...
// Kronecker Product
template <class T>
Matrix<T> Matrix<T>::kron(const Matrix<T> &m) const
{
      ...
      for (j = 0; j < colNum; j++)
         for (k = 0; k < m.rowNum; k++)
            for (p = 0; p < m.colNum; p++)
               result[k + i * m.rowNum][p + j * m.colNum] = mat[i][j] * m.mat[k][p];
   return result;
}
...
// Direct Sum
template <class T>
Matrix<T> Matrix<T>::dsum(const Matrix<T> &m) const
{
   int size1 = rowNum + m.rowNum,
       size2 = colNum + m.colNum;
   Matrix<T> result(size1, size2);
...
template <class T>
ostream &Matrix<T>::output(ostream &s) const
{
...
template <class T>
istream &Matrix<T>::input(istream &s)
{
   int i, j, num1, num2;
   s.clear(); // set stream state to good
   s >> num1; // read in row number
   if (!s.good())
      return s; // can't get an integer, just return
