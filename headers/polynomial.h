...
template <class T>
int Polynomial<T>::Karatsuba = 1;
// inversion by Newton iteration for division
template <class T>
int Polynomial<T>::Newton = 1;
...
{
  if (c != zero(T()))
    terms.push_back(pair<T, int>(c, 0));
}
...
Polynomial<T>::operator+(const Polynomial<T> &p) const
{
  ...
  if (constant())
    p2.variable = p.variable;
  else if (p.constant())
    p2.variable = variable;
  else
    assert((p2.variable = variable) == p.variable);

  while (i != terms.end() && j != p.terms.end())
  {
    if (i->second < j->second)
      p2.terms.push_back(*(j++));
    else if (i->second > j->second)
      p2.terms.push_back(*(i++));
    ...
    Polynomial<T>::operator*(const Polynomial<T> &p) const
{
  ...
  else // Karatsuba multiplication
  {
    ...
    while (degree <= degree1 || degree <= degree2)
      degree *= 2;
                                       ...
                                       const Polynomial<T> &p2, int n) const
{
  ...
  {
    Polynomial<T> p;
    if (p1.terms.empty() || p2.terms.empty())
      return p;
  ...
  // multiply by x^(n/2)
  for (j = r4.terms.begin(); j != r4.terms.end(); j++)
    j->second += n2;
  // multiply by x^n
  for (j = r1.terms.begin(); j != r1.terms.end(); j++)
    j->second += n;
                                 ...
                                 const Polynomial<T> &b) const
{
  ...
  if (c.terms.empty())
    return Polynomial<T>(one(T()));
  if (d.terms.empty())
    return Polynomial<T>(one(T()));

  c /= c.terms.front().first;
  d /= d.terms.front().first;
