    ...
    for (int r = rows() - 1; r >= 0; r--)
        for (int c = cols() - 1; c >= 0; c--)
            if (r == rows() - 1 && c == cols() - 1)
                pattern_match_OR(l, Matrix<Symbolic>::operator[](r)[c].match(sm->operator[](r)[c], p));
            else
                pattern_match_AND(l, Matrix<Symbolic>::operator[](r)[c].match(sm->operator[](r)[c], p));
    return l;
}
