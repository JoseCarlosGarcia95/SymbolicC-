...
public:
  ...
  Symbolic subst(const Symbolic &, const Symbolic &,
                 int &n = subst_count) const;
  ...
  Symbolic subst_all(const Symbolic &, const Symbolic &,
                     int &n = subst_count) const;
...
SymbolicInterface::SymbolicInterface() { simplified = expanded = 0; }
...
const type_info &SymbolicInterface::type() const { return typeid(*this); }
...
void SymbolicProxy::print(ostream &o) const { (*this)->print(o); }

const type_info &SymbolicProxy::type() const { return (*this)->type(); }

Symbolic SymbolicProxy::subst(const Symbolic &x, const Symbolic &y,
                              int &n) const {
...
Symbolic SymbolicProxy::df(const Symbolic &s) const { return (*this)->df(s); }
...
PatternMatches SymbolicProxy::match(const Symbolic &s,
                                    const list<Symbolic> &p) const {
  return (*this)->match(s, p);
}

PatternMatches SymbolicProxy::match_parts(const Symbolic &s,
                                          const list<Symbolic> &p) const {
...
Symbolic::Symbolic(const CloningSymbolicInterface &s) { *this = s; }
...
Symbolic::Symbolic(const int &i) : SymbolicProxy(Number<int>(i).simplify()) {}

Symbolic::Symbolic(const double &d)
    : SymbolicProxy(Number<double>(d).simplify()) {}

Symbolic::Symbolic(const string &s) : SymbolicProxy(Symbol(s).simplify()) {}

Symbolic::Symbolic(const char *s) : SymbolicProxy(Symbol(s).simplify()) {}
...
Symbolic Symbolic::operator[](const Equation &p) const { return subst(p); }

Symbolic Symbolic::operator[](const Equations &l) const { return subst(l); }
...
Symbolic Symbolic::subst_all(const Symbolic &x, const Symbolic &y,
                             int &n) const {
  int n1 = n;
...
Symbolic Symbolic::coeff(const int &i) const { return coeff(Number<int>(i)); }
...
Symbolic Symbolic::operator|(const Symbolic &s) const {
  if (rows() != s.rows() || columns() != s.columns() ||
      (rows() != 1 && columns() != 1)) {
    cerr << "Attempt to dot product " << *this << " and " << s << " failed."
         << endl;
...
Symbolic Symbolic::operator%(const Symbolic &s) const {
  if (rows() != s.rows() || columns() != s.columns() ||
      (rows() != 1 && columns() != 1)) {
    cerr << "Attempt to cross product " << *this << " and " << s << " failed."
         << endl;
    throw SymbolicError(SymbolicError::IncompatibleVector);
  }
...
Symbolic Symbolic::kron(const Symbolic &s) const {
  if (type() != typeid(SymbolicMatrix) || s.type() != typeid(SymbolicMatrix)) {
