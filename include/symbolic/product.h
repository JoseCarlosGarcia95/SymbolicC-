            ...
            Number<void>(p->parameters.back()).numerictype() == typeid(int))
        {
          ...
          if (n > 0)
          {
            if (p->parameters.front().type() == typeid(Product))
            {
              ...
              for (int m = 0; m < n; ++m)
                u.push_back(p->parameters.front());
          }
          else
            u.push_back(*i);
        }
            ...
            Number<void>(p->parameters.back()).numerictype() == typeid(int))
        {
          ...
          if (n > 0)
          {
            if (p->parameters.front().type() == typeid(Product))
            {
              ...
              for (int m = 0; m < n; ++m)
                v.push_back(p->parameters.front());
          }
          else
            v.push_back(*i1);
        }
...
Simplified Product::simplify() const
{
  ...
  if (!n.isOne())
    r.factors.push_front(n->simplify());
  if (r.factors.size() == 0)
    return Number<int>(1);
  if (r.factors.size() == 1)
    return r.factors.front();
  return r;
}
...
int Product::compare(const Symbolic &s) const
{
  ...
  return (subst(s, Symbolic(1), c) == 1 && s.subst(*this, Symbolic(1), c) == 1 && c == 2);
}
...
Symbolic Product::integrate(const Symbolic &s) const
{
  ...
  if (count == 1)
  {
    ...
    for (i = factors.begin(); i != factors.end(); ++i)
      if (i == i1)
        p.factors.push_back(::integrate(*i, s));
      else
        p.factors.push_back(*i);
    return p;
  }
...
Product::match(const Symbolic &s, const list<Symbolic> &p) const
{
  ...
  if (factors.size() == 0)
    return l;
  if (factors.size() == 1)
    return factors.front().match(s, p);
  if (s.type() != type())
    return l;
  ...
  for (j = thismatch.begin(); j != thismatch.end(); ++j)
  {
    ...
    for (k = j->begin(), i = prod->factors.begin();
      ...
      if (*k)
        p1.factors.push_back(*i);
      else
      {
        for (k1 = k, i1 = i; k1 != j->end() && i1 != prod->factors.end(); ++i1, ++k1)
          if ((*k ^ *k1) && !i->commute(*i1))
            break;
        if (k1 == j->end())
          p2.factors.push_back(*i);
        else
          break;
      }
...
#endif
    {
      if (p1.factors.size() != 0)
        pattern_match_OR(l, s.match(p1, p));
      if (p2.factors.size() != 0)
