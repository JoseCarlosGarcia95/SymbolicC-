...
Simplified Power::simplify() const {
  ...
  if (base.type() == typeid(Power)) {
    ...
    int i = CastPtr<const Number<int>>(exponent)->n;
    int inv = (i < 0) ? 1 : 0;
    Number<void> r = Number<int>(1), x = base;
    i = (inv) ? -i : i;
    while (i != 0) {
      if (i & 1)
        r = r * x;
    ...
    if (inv)
      return (Number<int>(1) / r);
    return r;
  } else if (exponent.type() == typeid(Log)) {
    CastPtr<const Log> l(exponent);
    if (l->parameters.front() == base)
      return l->parameters.back();
  } else if (base.type() == typeid(Numeric) &&
             exponent.type() == typeid(Numeric) &&
             Number<void>(exponent).numerictype() ==
                 typeid(Rational<Number<void>>)) {

    CastPtr<const Number<Rational<Number<void>>>> rat(exponent);

    if (Number<void>(rat->n.num()).numerictype() == typeid(int) &&
        Number<void>(rat->n.den()).numerictype() == typeid(int)) {
      Symbolic ifactor(1), ofactor(1);

      CastPtr<const Number<int>> inum = rat->n.num();
      CastPtr<const Number<int>> iden = rat->n.den();

      int num = inum->n;
      int den = iden->n;
      if (Number<void>(base).numerictype() == typeid(int)) {
        CastPtr<const Number<int>> ibase(base);

        list<int> factors = list<int>{};
        int iibase = int(ibase->n);

        _factorize_trial_division<int>(iibase, &factors);

        std::list<int>::iterator i = factors.begin();

        while (i != factors.end()) {
          int factor = (*i);
          int times = 0;

          for (auto test : factors) {
            if (test == factor) {
              i++;
              times++;
            }
    ...
    double nd, bd = CastPtr<const Number<double>>(base)->n;
    if (Number<void>(exponent).numerictype() == typeid(int))
      nd = CastPtr<const Number<int>>(exponent)->n;
    else if (Number<void>(exponent).numerictype() == typeid(double))
      nd = CastPtr<const Number<double>>(exponent)->n;
    else if (Number<void>(exponent).numerictype() ==
             typeid(Rational<Number<void>>))
      nd = double(CastPtr<const Number<Rational<Number<void>>>>(exponent)->n);
    else
    ...
    if (bd >= 0.0 || int(nd) == nd)
      return Number<double>(pow(bd, nd));
             ...
             exponent.type() == typeid(Numeric) &&
             Number<void>(exponent).numerictype() == typeid(double)) {
    double bd, nd = CastPtr<const Number<double>>(exponent)->n;
    if (Number<void>(base).numerictype() == typeid(int))
      bd = CastPtr<const Number<int>>(base)->n;
    else if (Number<void>(base).numerictype() == typeid(double))
      bd = CastPtr<const Number<double>>(base)->n;
    else if (Number<void>(base).numerictype() == typeid(Rational<Number<void>>))
      bd = double(CastPtr<const Number<Rational<Number<void>>>>(base)->n);
    else
      return Power(base, exponent);
    if (bd >= 0.0 || int(nd) == nd)
      return Number<double>(pow(bd, nd));
...
Expanded Power::expand() const {
  ...
  if (b.type() != typeid(Sum) && b.type() != typeid(Product) &&
  ...
  while (i != 0) {
    if (i & 1)
      r = Product(r, x).expand();
    i >>= 1;
    if (i != 0)
      x = Product(x, x).expand();
  }
...
PatternMatches Power::match_parts(const Symbolic &s,
  ...
  if (parameters.back().type() == typeid(Numeric) &&
    ...
    Product pr;
    while (n-- > 0)
      pr.factors.insert(pr.factors.end(), parameters.back());
...
Derivative::Derivative(const Symbolic &s1, const Symbolic &s2) : Symbol("df") {
...
int Derivative::compare(const Symbolic &s) const {
  ...
  if (d->parameters.size() != parameters.size())
    return 0;
  if (d->parameters.front() != parameters.front())
    return 0;
  for (i = parameters.begin(), ++i; i != parameters.end(); ++i) {
    for (j = d->parameters.begin(), ++j; j != d->parameters.end(); ++j)
      if (*i == *j)
        break;
    if (j == d->parameters.end())
      return 0;
...
Symbolic Integral::df(const Symbolic &s) const {
  ...
  if (i1 != parameters.end()) {
    ...
    if (in->parameters.size() == 1)
      return in->parameters.front();
    return *in;
  }
...
Simplified Kronecker::simplify() const {
  ...
  if (s1.type() != typeid(SymbolicMatrix))
    return Kronecker(s1, s2);
  if (s2.type() != typeid(SymbolicMatrix))
    return Kronecker(s1, s2);
  return SymbolicMatrix(CastPtr<const SymbolicMatrix>(s1)->kron(
      *CastPtr<const SymbolicMatrix>(s2)));
}
...
Simplified DirectSum::simplify() const {
  ...
  const Symbolic &s2 = parameters.back().simplify();
  if (s1.type() != typeid(SymbolicMatrix))
    return DirectSum(s1, s2);
  if (s2.type() != typeid(SymbolicMatrix))
    return DirectSum(s1, s2);
  return SymbolicMatrix(CastPtr<const SymbolicMatrix>(s1)->dsum(
      *CastPtr<const SymbolicMatrix>(s2)));
}
...
Simplified Hadamard::simplify() const {
  ...
  const Symbolic &s2 = parameters.back().simplify();
  if (s1.type() != typeid(SymbolicMatrix))
    return Hadamard(s1, s2);
  if (s2.type() != typeid(SymbolicMatrix))
    return Hadamard(s1, s2);
  return SymbolicMatrix(CastPtr<const SymbolicMatrix>(s1)->hadamard(
      *CastPtr<const SymbolicMatrix>(s2)));
}
