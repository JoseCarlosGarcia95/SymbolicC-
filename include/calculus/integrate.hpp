...
Symbolic integrate(const Symbolic &, const Symbolic &, const Symbolic &,
                   const Symbolic &);
  ...
  for (i = eq.begin(); i != eq.end(); ++i)
    try {
      if (df(rhs(*i, a), x) == 0 && df(rhs(*i, c), x) == 0) {
        ...
        if (df(bp, x, 5) == 0)
          return f / (ap + cp) -
                 integrate(df(bp, x) * exp((ap + cp) * x) / (ap + cp), x);
      }
    } catch (const SymbolicError &se) {
    }
...
Symbolic integrate(const Symbolic &f, const Symbolic &x, const Symbolic &a,
                   const Symbolic &b) {
